...
my $posts = Kalaclista::Constants->rootdir(qr{^t$})->child('public/dist')->path;

my $parser = HTML5::DOM->new( { scripts => 1 } );

sub main {
  my @files = grep { $_ =~ m{(nyarla|licenses|policies)/index\.html} } Kalaclista::Files->find($posts);
  ...
  for my $path (@files) {
    ...
    is( $dom->at('meta[property="og:section"]')->getAttribute('content'),           'pages' );
